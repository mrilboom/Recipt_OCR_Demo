...
def run(MODEL_DB_PATH, MODEL_CRNN_PATH, IMGPATH,L:list):
    a = time.time()
    print(IMGPATH)
    OUT.write(IMGPATH)
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(DIR):
        for file in files:
            ...
            if file.split(".")[-1] == "jpg":
                ...
                O = run(MODEL_DB_PATH, MODEL_CRNN_PATH, path.replace('\\','/'))
                O['图像路径']=path.replace('\\','/')
                LL.append(O)
                print("***********")
                OUT.write("---------------\n")
    ...
    import xlwt
    exl = xlwt.Workbook()
    sheet = exl.add_sheet("识别结果")
    # DB init
    for i,item in enumerate(L):
        sheet.write(0,i,item)
    for i,O in enumerate(LL,1):
        for key,value in O.items():
            # print(str(i)+"     "+key+"   "+value)
            sheet.write(i,L.index(key),value)
        sheet.write(i,len(L), IMAPATH)
    exl.save("out.xls")
